...
const columns = [
  ...
  columnHelper.accessor("Name", {
    ...
    header: () => (
      <span className="flex items-center">
        <Mail className="mr-2" size={16} /> Name
      </span>
    ),
  }),
  columnHelper.accessor("Feedback", {
    ...
    header: () => (
      <span className="flex items-center">
        <Mail className="mr-2" size={16} /> Feedback
      </span>
    ),
  }),
...
function Feedback(){
  ...
  const [pagination, setPagination] = React.useState({
    ...
    useEffect(() => {
      ...
      const dbRef = ref(database, "ReportRecord");
      const unsubscribe = onValue(dbRef, (snapshot) => {
        if (snapshot.exists()) {
          const usersData = snapshot.val();
          const formattedData = Object.values(usersData).map((ReportRecord) => ({
            UserId: ReportRecord.UserID,
            Date: ReportRecord.Date_and_Time_Submitted            ,
            Name: ReportRecord.Name,
            Feedback: ReportRecord.User_Report
          }));
  ...
  console.log(table.getRowModel());
    ...
    return(
      ...
      <h2 className= "font-nobile text-[#1c2e8b] text-3xl font-medium"> User's Feedback</h2>
        </div>
